...
const downloadFile = (req, res) => {
   ...
   if (downloadData) {
    ...
    /*  const fileContent = responseData.Records.map(item => `P000284|S000742|${item.SALETYPE[0]=='SALE' ? 'B' : item.SALETYPE[0]=='RETURN' ? 'N' : 'B'}|${item.RECEIPTID[0] ? item.RECEIPTID[0] : 'nil'}|${item.TRANSDATE[0]}|${item.QTY[0]}|${item.INVOICEAMOUNT[0]}|${item.DISCOUNT[0]}|${item.NETAMOUNT[0]}|${item.TAXAMOUNT[0]}|${item.SALETYPE[0]=='SALE' ? 'N' : item.SALETYPE[0]=='RETURN' ? 'N' : 'C'}|PAID-I|${formattedDate}`).join('\n'); */
...
module.exports = {getAllBills, downloadFile,ping}
